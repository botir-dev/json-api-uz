...
export default function NotificationsPage() {
  ...
  const loadTemplates = async () => {
    try {
      setTemplates(await notifications.listTemplates());
    } catch {}
  };
  useEffect(() => {
    loadLogs(1);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSendErr(e instanceof Error ? e.message : "Xato");
    }
    setSending(false);
  };
  ...
  return (
    <div className="p-6 lg:p-8 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>
            Email, SMS va push bildirishnomalarini boshqaring
          </p>
        </div>
      ...
      {toast && (
        <div
          className="card p-4 mb-5 text-sm text-emerald-500 animate-slide-up"
          style={{
                ...
                {logs.map((log) => (
                  <tr key={log.id} className="table-row-hover">
                    <td className="table-cell">
                      <span className="badge-gray">{log.channel}</span>
                    </td>
                    <td
                      className="table-cell font-mono text-xs max-w-[200px] truncate"
                    ...
                    <td className="table-cell">
                      <span
                        className={STATUS_BADGE[log.status] || "badge-gray"}
                      >
                        {log.status}
                      </span>
                    </td>
          ...
          <form onSubmit={handleSend} className="space-y-4">
            <div className="space-y-1">
              ...
              <input
                ...
                value={sendForm.to}
                onChange={(e) =>
                  setSendForm((f) => ({ ...f, to: e.target.value }))
                }
              ...
              {templates.length > 0 && (
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSendForm((f) => ({ ...f, template: e.target.value }))
                    }
            ...
            <div className="space-y-1">
              ...
              <input
                type="text"
                placeholder="Bildirishnoma mavzusi"
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                value={sendForm.body}
                onChange={(e) =>
                  setSendForm((f) => ({ ...f, body: e.target.value }))
                }
        ...
        <Modal title="Shablon yaratish" onClose={() => setShowTemplate(false)}>
          <form onSubmit={handleTemplate} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={tmplForm.name}
                  onChange={(e) =>
                    setTmplForm((f) => ({ ...f, name: e.target.value }))
                  }
              ...
              <div className="space-y-1">
                <label className="label">Kanal</label>
                <select
                  value={tmplForm.channel}
                  onChange={(e) =>
                    setTmplForm((f) => ({ ...f, channel: e.target.value }))
                  }
                  className="input-dark"
                >
                  <option value="email">Email</option>
            ...
            <div className="space-y-1">
              ...
              <input
                type="text"
                placeholder="Xush kelibsiz, {{name}}!"
                value={tmplForm.subject}
                onChange={(e) =>
                  setTmplForm((f) => ({ ...f, subject: e.target.value }))
                }
                className="input-dark"
              />
            </div>
            <div className="space-y-1">
              ...
              <textarea
                placeholder="<p>Salom {{name}}, xush kelibsiz!</p>"
                value={tmplForm.bodyHtml}
                onChange={(e) =>
                  setTmplForm((f) => ({ ...f, bodyHtml: e.target.value }))
                }
