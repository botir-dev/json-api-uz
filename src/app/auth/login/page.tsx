...
export default function LoginPage() {
  ...
  const saveAndRedirect = (accessToken: string, refreshToken: string | undefined, slug: string) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.accessToken) {
        saveAndRedirect(data.accessToken, data.refreshToken, form.tenantSlug.trim());
      } else {
        setError("Server noto'g'ri javob qaytardi. Qayta urinib ko'ring.");
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Xato yuz berdi";
      setError(msg.includes("Invalid credentials") ? "Email, parol yoki tenant slug noto'g'ri" : msg);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="my-auto space-y-6">
          <blockquote className="text-4xl font-display leading-tight text-white">
            &ldquo;Platformangiz<br />
            uchun <em className="italic text-zinc-400">kuchli</em><br />
            backend.&rdquo;
          </blockquote>
          <p className="text-zinc-500 text-sm max-w-xs">
            Kolleksiyalar, API kalitlar, webhooklar va analitikani bir joyda boshqaring.
          </p>
        </div>
      ...
      <div className="min-h-screen flex">
        ...
        <div className="flex-1 flex items-center justify-center bg-zinc-50 px-8">
          <div className="w-full max-w-sm space-y-8">
            <div>
              <h1 className="font-display text-3xl font-bold text-zinc-900">2FA Tasdiqlash</h1>
              <p className="text-zinc-500 text-sm mt-2">Autentifikator ilovangizdan 6 xonali kodni kiriting</p>
            </div>
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-zinc-50 px-8 py-12">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div>
            <h1 className="font-display text-3xl font-bold text-zinc-900">Xush kelibsiz</h1>
            <p className="text-zinc-500 text-sm mt-1.5">Davom etish uchun akkauntingizga kiring</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1.5">
              <label className="auth-label">TENANT SLUG <span className="text-red-400">*</span></label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setForm(f => ({ ...f, tenantSlug: slugify(e.target.value) }))}
              ...
              <p className="text-zinc-400 text-xs">Ro&apos;yxatdan o&apos;tganda ko&apos;rsatilgan slug</p>
            </div>
          ...
          <p className="text-center text-zinc-500 text-sm">
            Akkaunt yo&apos;qmi?{" "}
            <Link href="/auth/register" className="text-zinc-900 font-semibold underline underline-offset-2 hover:text-zinc-600 transition-colors">
              Yaratish
            </Link>
          </p>
        </div>
...
function Spinner({ dark = true }: { dark?: boolean }) {
  return (
    <span className={`inline-block w-4 h-4 border-2 rounded-full animate-spin ${dark ? "border-zinc-300 border-t-zinc-900" : "border-white/30 border-t-white"}`} />
  );
}
